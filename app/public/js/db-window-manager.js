(() => {
    ...
    const eligiblePath = p => p === '/auftraege' || p.startsWith('/accounting-berichte') || p.startsWith('/kunden') || p === '/dokumentation' || p === '/handbuch' || p === '/sql-wiki' || p.startsWith('/rechnungen') || p === '/wiedervorlagen' || p === '/fremdaccounting' || p.startsWith('/datev') || p.startsWith('/rechnungslauf') || p.startsWith('/produkte') || p.startsWith('/staffelgruppen') || p.startsWith('/branchen-auswertung') || p.startsWith('/rechnungen-ohne-ust') || p.startsWith('/lastschriften');
