(() => {
    ...
    const eligiblePath = p => p.startsWith('/admin') || p === '/auftraege' || p.startsWith('/fakturierung') || p.startsWith('/accounting-berichte') || p.startsWith('/anbindungen') || p.startsWith('/netze') || p.startsWith('/ports') || p.startsWith('/dialins') || p.startsWith('/kunden') || p === '/dokumentation' || p === '/handbuch' || p === '/sql-wiki' || p.startsWith('/rechnungen') || p === '/wiedervorlagen' || p === '/fremdaccounting' || p.startsWith('/datev') || p.startsWith('/domainkonditionen') || p.startsWith('/domain-eintraege') || p.startsWith('/domain-zu-kunden') || p.startsWith('/domain-zu-auftragsposition') || p.startsWith('/ipv4-reverse') || p.startsWith('/domain-eintragen') || p.startsWith('/rechnungslauf') || p.startsWith('/produkte') || p.startsWith('/staffelgruppen') || p.startsWith('/sms-zugaenge') || p.startsWith('/linearstaffeln') || p.startsWith('/bandbreitentarife') || p.startsWith('/bereichsstaffeln') || p.startsWith('/zeittarife') || p.startsWith('/branchen-auswertung') || p.startsWith('/rechnungen-ohne-ust') || p.startsWith('/lastschriften');
    const navWords = /^(←\s*)?(zurück|abbrechen|hauptmenü|aufträge)$/i;
    ...
    window.openDbWindow = (href, title='Fenster', opts={}) => {
        ...
        const defaultWidth = isInvoice ? 'calc(100vw - 32px)' : (isConnections ? 'min(1120px,94vw)' : null);
        const defaultHeight = isInvoice ? 'calc(100vh - 78px)' : (isConnections ? 'min(860px,90vh)' : (isDialins ? 'min(920px,94vh)' : null));
    ...
    readState().forEach(x => {
        try {
            ...
            openDbWindow(x.href,x.title,{
                ...
                width:invoice ? 'calc(100vw - 32px)' : (connection ? 'min(1120px,94vw)' : x.width),
                height:invoice ? 'calc(100vh - 78px)' : (connection ? 'min(860px,90vh)' : (dialin ? 'min(920px,94vh)' : x.height)),
                min:x.min, max:x.max
            });
        } catch {}
    });
