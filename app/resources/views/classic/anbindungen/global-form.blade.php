...
<div class="subrow"><span>ID der Auftragsposition:</span><div class="readonly-filter" tabindex="0" data-filter-field="position_id" data-filter-value="{{ $position->intID }}">{{ $position->intID }}</div></div>
...
<div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:6px"><a class="button" href="{{ route('kunden.auftraege.positionen.edit',[$kunde->intID,$auftrag->intAufNr,$position->intID]) }}">Position öffnen</a><a class="button" href="{{ route('kunden.auftraege.show',[$kunde->intID,$auftrag->intAufNr]) }}">Auftrag öffnen</a></div></div></div>
