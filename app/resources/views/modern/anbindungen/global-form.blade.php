...
body{font:14px "Segoe UI",Arial;background:#f4f6f8;color:#1f2937;padding:26px}.window{max-width:1150px;margin:auto;background:#fff;border:1px solid #d1d5db;border-radius:10px;padding:20px}.top{display:flex;justify-content:space-between}.grid{display:grid;grid-template-columns:190px 1fr;gap:8px 10px;align-items:center}.button{display:inline-block;padding:7px 12px;border:1px solid #777;background:#eee;color:#111;text-decoration:none;cursor:pointer}input,select,textarea{width:100%;box-sizing:border-box;padding:6px;border:1px solid #999;background:#fff}textarea{min-height:90px}.info{margin-top:12px;padding:10px;border:1px solid #aaa;background:#fff}.err{background:#fee;border:1px solid #b91c1c;padding:8px}.ok{background:#e8f6e8;border:1px solid #4b8b4b;padding:8px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #bbb;padding:5px}th{background:#ddd}</style></head><body>
<x-page-help title="Verbindungen / Anbindungen">Hier werden technische Anbindungen und Accounting-Zuordnungen gepflegt. Änderungen können Erfassung und spätere Abrechnung beeinflussen. Vor dem Speichern Ziel, Kunde und Zuordnung prüfen.</x-page-help>
<div class="window"><div class="top"><h2>{{ $id ? 'Anbindungs-ID '.$id : 'Neue Anbindung' }}</h2><a href="{{ route('frontend.switch','classic') }}">← Zur klassischen Ansicht</a></div>
