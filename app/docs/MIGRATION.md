...
Das Rollback-/Notfallverfahren ist ebenfalls im Adminbereich dokumentiert. `INVOICE_WRITES_ENABLED` und `DUNNING_WRITES_ENABLED` sind aktuell explizit als `false` in der lokalen, nicht versionierten `/srv/dbapp/app/.env` hinterlegt. Nach einer späteren Produktivfreigabe dienen sie als getrennte Not-Aus-Schalter. Für einen Notfall werden die Flags auf `false` gesetzt, `php artisan config:clear` im App-Container ausgeführt und `dbapp-app` neu gestartet. Rechnungsschreiben verwendet eine SQL-Transaktion; bereits erzeugte PDF-/XML-Dokumente werden bei einem fehlgeschlagenen Commit gelöscht und Cleanup-Fehler separat geloggt. Vor jeder manuellen Korrektur ist eine frische CARDEA-Sicherung vorgesehen; ein vollständiger Restore bleibt größeren Datenbankschäden und einem kontrollierten Wartungsfenster vorbehalten.

Die technische Vorbereitung für die erste kontrollierte Produktivrechnung wurde am 19.09.2026 abgeschlossen. Als Abnahmefall ist Auftrag `5946` / Kunde `6432` vorgesehen, Zeitraum 01.09.–30.09.2026, Rechnungsdatum 19.09.2026, **ohne Accounting-Auswertung**. Erwartet werden eine Rechnungszeile, 9,90 € netto, 1,88 € Steuer und 11,78 € brutto. Der Auftrag ist ohne Warnung fakturierbar, kein Lastschriftfall und vollständig ZUGFeRD-fähig. Der exakte Test bestand XML-Schema, EN16931 und veraPDF PDF/A-3u. Die Rechnungsnummernsimulation meldet `2026001465` als nächste Nummer bei 0 Duplikaten und 0 Datums-/Nummernpräfixabweichungen. SQL-Schreibrechte, Janus-Rechnungsablage und PDF-Renderer sind vollständig bereit; beide produktiven Write-Schalter bleiben noch deaktiviert.
