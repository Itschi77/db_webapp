...
Vor der Produktivfreigabe wurde außerdem eine manuelle SQL-Backupfunktion in `/admin` ergänzt. Die Sicherungen laufen asynchron über einen eigenen Queue-Worker und werden nicht in der Rechnungsablage, sondern in der separaten Freigabe `\\janus\Backups` gespeichert. Der Dienst prüft vor dem Start Datenbankberechtigungen, die Berechtigung zum serverseitigen Lesen der erzeugten Backup-Dateien und den verfügbaren Speicherplatz. Große Backups werden in mehrere SQL-Backupgeräte gestript. Mit der aktuellen 50-GB-Systemplatte von Janus ist `accountings` wegen seiner letzten Full-Backup-Größe von rund 82,3 GB bewusst noch nicht freigegeben; für den vollständigen Produktiv-Backupweg muss die Backup-Ablage auf mindestens ausreichend große separate Kapazität verschoben oder Janus erweitert werden.
