...
Für die abschließende read-only Abnahme wurde im Adminbereich zusätzlich der **Rechnungstool-Systemtest** ergänzt. Er sucht im gewählten Zeitraum reale Vertreter für Festpreis, Voraus, Staffel/Accounting, Domain, Mehrpositionen und Rabatt, führt dieselbe Berechnungs-/Konsistenzlogik wie die Einzelvorschau aus und rendert für erfolgreiche Fälle eine temporäre PDF über die produktive Word/LibreOffice-Strecke. Zusätzlich werden `\\janus\Rechnungen`, der read-only MIDAS-Altpfad, der deaktivierte Produktivschreibschalter und der PDF-Renderer geprüft. Der Lauf schreibt weder Rechnungen noch Abrechnungsdaten. Nach Korrektur der Kandidatensuche und Aktivierung der Accounting-Auswertung für Domainfälle bestand der Lauf vom 19.09.2026 alle sechs Fallklassen und alle vier Systemprüfungen.
...
Die Vorschau erzeugt weiterhin ein nicht gespeichertes A4-PDF mit Wasserzeichen. Verbindlich erzeugte Rechnungen werden dagegen ausschließlich in der lokalen Janus-Ablage `/mnt/rechnungen` gespeichert. Das aktive Adminprofil `storage.rechnungen` verwaltet lokalen Mount, UNC-Wurzel `\\janus\Rechnungen` und Dateinamenschema; aktuell wird `{year}/{invoice_number}.pdf` verwendet. Der zugrunde liegende Hostordner `/srv/dbapp/data/rechnungen` wird von einem eigenen Samba-Docker-Service als schreibbare Freigabe `Rechnungen` bereitgestellt. Der Schreibtest prüft sowohl den Laravel-Speicherpfad als auch den SMB-Zugriff. MIDAS bleibt für historische Dokumente read-only und ist für neue Dokumente ausdrücklich kein Schreibziel.

Der Adminbereich verwaltet den späteren Rechnungsversand über das Profil `smtp.primary`. Konfigurierbar sind SMTP-Host und Port, Postfach/Anmeldename, verschlüsselt gespeichertes Passwort, STARTTLS/SMTPS/keine Verschlüsselung, Anmeldung mit Postfach/Passwort oder ohne Authentifizierung sowie Absenderadresse und -name. Der einfache Test prüft nur die Erreichbarkeit; ein separater bestätigungspflichtiger Testversand prüft Anmeldung und Zustellung mit der vorbelegten Zieladresse `sw@tops.net`. Jeder Versuch wird ohne Passwort im Laravel-Log protokolliert.

Vor der Produktivfreigabe wurde außerdem eine manuelle SQL-Backupfunktion in `/admin` ergänzt. Die Sicherungen laufen asynchron über einen eigenen Queue-Worker und werden ausschließlich direkt auf CARDEA im SQL-Server-Standard-Backupordner erzeugt. Eine Kopie der `.bak`-Dateien nach Janus findet nicht statt. Der Adminbereich prüft SQL-Verbindung, `BACKUP DATABASE` und Zielpfad und zeigt die letzten Vollsicherungen direkt aus der SQL-Server-Backup-Historie (`msdb`) an. Die frühere Zwischenlösung mit `\\janus\Backups`, Binärtransfer über `OPENROWSET` und `ADMINISTER BULK OPERATIONS` wurde wieder entfernt.
...
Im selben Adminblock wurde außerdem ein **Mahnwesen-Endtest** ergänzt. Der Lauf arbeitet ausschließlich lesend mit realen offenen Rechnungen und deckt Mahnstufen 1 bis 3, Summenbildung aus Hauptbetrag/Gebühren/Zinsen, strittige Fälle, Wiedervorlage, Ratenzahlung, Kundensperre und Mahnschreiben-PDF ab. Zusätzlich wird der produktive Guard durch einen absichtlich aufgerufenen Schreibpfad verifiziert; bei `DUNNING_WRITES_ENABLED=false` muss dieser vor jedem SQL-Write abbrechen. Der bestätigte Lauf vom 19.09.2026 bestand 9 von 9 Prüfungen.

Das Rollback-/Notfallverfahren ist ebenfalls im Adminbereich dokumentiert. Seit der kontrollierten Freigabe am 19.09.2026 steht `INVOICE_WRITES_ENABLED=true`, während `DUNNING_WRITES_ENABLED=false` bleibt. Beide Werte liegen ausschließlich in der lokalen, nicht versionierten `/srv/dbapp/app/.env` und dienen als getrennte Not-Aus-Schalter. Für einen Notfall werden die betroffenen Flags auf `false` gesetzt; falls ein Laravel-Config-Cache vorhanden ist, wird dieser geleert und der App-Prozess anschließend neu geladen. Rechnungsschreiben verwendet eine SQL-Transaktion; bereits erzeugte PDF-/XML-Dokumente werden bei einem fehlgeschlagenen Commit gelöscht und Cleanup-Fehler separat geloggt. Vor jeder manuellen Korrektur ist eine frische CARDEA-Sicherung vorgesehen; ein vollständiger Restore bleibt größeren Datenbankschäden und einem kontrollierten Wartungsfenster vorbehalten.

Die technische Vorbereitung für die erste kontrollierte Produktivrechnung wurde am 19.09.2026 abgeschlossen. Als Abnahmefall ist Auftrag `5946` / Kunde `6432` vorgesehen, Zeitraum 01.09.–30.09.2026, Rechnungsdatum 19.09.2026, **ohne Accounting-Auswertung**. Erwartet werden eine Rechnungszeile, 9,90 € netto, 1,88 € Steuer und 11,78 € brutto. Der Auftrag ist ohne Warnung fakturierbar, kein Lastschriftfall und vollständig ZUGFeRD-fähig. Der exakte Test bestand XML-Schema, EN16931 und veraPDF PDF/A-3u. Die Rechnungsnummernsimulation meldet `2026001465` als nächste Nummer bei 0 Duplikaten und 0 Datums-/Nummernpräfixabweichungen. SQL-Schreibrechte, Janus-Rechnungsablage und PDF-Renderer sind vollständig bereit. Der Rechnungsschreibpfad wurde anschließend kontrolliert freigegeben; das Mahnwesen bleibt weiterhin deaktiviert. Zu diesem Zeitpunkt wurde noch keine Rechnung erzeugt.

Direkt vor diesem Freigabestand wurden über den produktiven Backup-Worker drei neue CARDEA-COPY_ONLY-Backups mit CHECKSUM und COMPRESSION erstellt und anschließend in `msdb` verifiziert. `accountings` lief von 22:45:40 bis 22:49:55 und wurde auf rund 24,31 GB komprimiert; `domains` und `topsnetdb_safe` wurden unmittelbar danach erfolgreich gesichert. Damit lag für den kontrollierten Erstlauf ein eindeutiger gemeinsamer Pre-Release-Backupstand vor.

Der erste produktive Schreibvorgang erzeugte Rechnung `2026001465` / interne ID `148798` für Auftrag `5946`. SQL-Buchung, Beträge, Positionszeile und Nummernkreis waren korrekt. Die anschließende PDF-Prüfung zeigte jedoch einen Stammdatenfehler: In `topsnetdb_safe.dbo.tblKunde` und `accountings.dbo.tblRechnungsanschrift` waren bei Kunde `6432` PLZ und Ort vertauscht (`Erpel` / `53579`). Der Rechnungsschreibschalter wurde daraufhin sofort wieder deaktiviert. Ein gezielter DELETE-Rollback über `janus_connect` wurde bewusst nicht durch zusätzliche Rechte erzwungen; die vorhandenen Minimalrechte verweigerten DELETE auf `tblAuftragPosBerechnet`, wodurch die Rollback-Transaktion vollständig unverändert zurücklief. Der fehlerhafte PDF-Beleg wurde separat außerhalb der Rechnungsfreigabe archiviert.

Nach Korrektur der beiden Stammdatensätze auf `53579` / `Erpel` wurde die bestehende Rechnung mit derselben Rechnungsnummer aus dem historischen September-Berechnungsstand neu gerendert und das PDF ersetzt. Die gespeicherte Positionsbuchung stimmt mit der historischen Neuberechnung bei Position, Berechnungsdatum, Netto und Steuer exakt überein; die korrigierte PDF enthält `53579 Erpel` und bestand ZUGFeRD-XSD, EN16931 und veraPDF PDF/A-3u erneut ohne Fehler.

Als Folgemaßnahme wurde eine zentrale Rechnungsanschrift-Plausibilitätsprüfung ergänzt. Sie blockiert den Auftrag, wenn das Feld `strPLZ` Text enthält und `strOrt` ausschließlich numerisch ist. Dieselbe Regel wurde in die tägliche Kunden-/Rechnungsanschrift-Konsistenzprüfung integriert. Bei Einführung blieben zwei aktive Rechnungstool-Aufträge mit diesem Muster übrig: Auftrag `5602` / Kunde `6342` und Auftrag `5926` / Kunde `6426`; deren Stammdaten werden nicht automatisch verändert.
