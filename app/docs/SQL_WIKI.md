...
Historische Pfade folgen weiterhin `\\midas\bh\Rechnungswesen\<Jahr>\Rechnungen\Papier\<Rechnungsnummer>.doc`; diese Altfreigabe bleibt ausschließlich read-only. Für neue Dokumente verwendet Janus das Dateisystemprofil `storage.rechnungen` mit lokalem Mount `/mnt/rechnungen`, UNC-Wurzel `\\janus\Rechnungen` und Muster `{year}/{invoice_number}.pdf`. Der Hostordner liegt unter `/srv/dbapp/data/rechnungen` und wird von einem eigenen Samba-Container freigegeben. Der SMB-Zugriff ist ausschließlich authentifiziert über den Benutzer `rechnungen`; Gastzugriff ist deaktiviert. Das Kennwort wird aus der nicht versionierten Datei `/srv/dbapp/secrets/samba.env` geladen.
...
Der Altcode setzte `boolSEPA` nach dem Lesen temporär immer auf wahr und schrieb `datVersendeDatum` bereits bei der Rechnungserzeugung. Diese beiden unsicheren Verhaltensweisen werden nicht übernommen: Ohne SEPA-Freigabe wird blockiert, und ein Versanddatum darf erst nach einer tatsächlich ausgeführten Versandaktion gesetzt werden.
