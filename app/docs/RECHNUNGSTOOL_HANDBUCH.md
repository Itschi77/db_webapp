...
### Endtest Rechnungstool

Der Endtest wurde bewusst aus der normalen Fakturierungsansicht in den **Adminbereich** verschoben. Dort steht er als **Rechnungstool-Systemtest** für die letzte read-only Abnahme vor der produktiven Freigabe zur Verfügung. Zeitraum und Rechnungsdatum sind frei wählbar. Der Lauf sucht automatisch reale Aufträge für sechs Fallklassen: normale Festpreisrechnung, Vorausberechnung, Staffel/Accounting, Domainrechnung, mehrere Dokumentpositionen und Rabatt.

Für jede Fallklasse werden bis zu 150 passende Kandidaten geprüft, bis ein vollständig fakturierbarer Auftrag gefunden wird. Domainfälle werden dabei mit aktivierter Accounting-/Staffelauswertung geprüft. Der gewählte Auftrag durchläuft dieselbe Berechnungs- und Konsistenzlogik wie die Einzelvorschau. Anschließend wird serverseitig eine echte PDF-Vorschau über die produktive Word-Vorlage und LibreOffice erzeugt und strukturell als PDF geprüft. Die Datei wird nur temporär erzeugt und danach wieder gelöscht; es wird keine Rechnung gespeichert, keine Rechnungsnummer reserviert und keine Abrechnung verbucht. Beim Mehrpositionsfall müssen tatsächlich mindestens zwei Dokumentzeilen entstehen. Der Rabattfall gilt nur dann als bestanden, wenn im erzeugten Dokument eine echte Zeile vom Typ **Rabatt** enthalten ist.

Zusätzlich prüft der Endtest die Systemvoraussetzungen: aktive Rechnungsablage ausschließlich auf `\\janus\Rechnungen`, `storage.midas` im Modus read-only, weiterhin deaktiviertes `INVOICE_WRITES_ENABLED` und die Verfügbarkeit von Word-Vorlage und LibreOffice-PDF-Konvertierung. Jeder Prüfschritt wird einzeln mit **OK**, **Kein Fall** oder **Fehler** angezeigt. Gefundene Aufträge sind direkt anklickbar.

Ein fehlgeschlagener Falltest bedeutet nicht automatisch einen Programmfehler. Der Endtest verwendet reale Aufträge und meldet deshalb auch fachlich blockierende Bestandsdaten, beispielsweise widersprüchliche Bankeinzugskennzeichen, fehlende Rechnungs-E-Mail-Adressen oder einen im gewählten Zeitraum nicht tatsächlich wirksamen Rabatt. Solche Fälle müssen vor der Produktivfreigabe entweder fachlich bereinigt oder durch einen anderen repräsentativen Zeitraum/Fall abgedeckt werden.
...
Der exakt konfigurierte ZUGFeRD-Abnahmelauf für diesen Auftrag bestand XSD, EN16931 und PDF/A-3u mit veraPDF 1.30.2 ohne Fehler. Unmittelbar vor der Freigabe wurden auf CARDEA neue COPY_ONLY-Backups aller drei produktiven Datenbanken erstellt und in `msdb` bestätigt: `accountings` (`DB-Webapp_accountings_20260919_204540.bak`), `domains` (`DB-Webapp_domains_20260919_204955.bak`) und `topsnetdb_safe` (`DB-Webapp_topsnetdb_safe_20260919_204955.bak`). Nach Abschluss dieses Vorbereitungsschritts wurde das Rechnungsschreiben kontrolliert freigegeben. Das Mahnwesen bleibt deaktiviert.

Die erste produktive Rechnung wurde anschließend als `2026001465` / interne ID `148798` für Auftrag `5946` erzeugt. Die Buchung selbst war betrags- und zeilengleich zum Vorabtest, jedoch zeigte die PDF-Prüfung einen Stammdatenfehler: In Kundenstamm und Rechnungsanschrift waren PLZ und Ort vertauscht (`Erpel` / `53579`). Der Schreibschalter wurde sofort wieder deaktiviert. Nach Korrektur der beiden Stammdatensätze wurde die Rechnung mit **derselben Rechnungsnummer** erneut aus dem historisch gebuchten September-Zustand gerendert und die bestehende PDF ersetzt. Die korrigierte Datei enthält `53579 Erpel`, stimmt bei Position, Berechnungsdatum, Netto, Steuer und Brutto exakt mit der gespeicherten Buchung überein und bestand erneut ZUGFeRD-XSD, EN16931 sowie PDF/A-3u mit veraPDF 1.30.2. Der fehlerhafte Originalbeleg wurde außerhalb der Rechnungsfreigabe als Rollback-Nachweis archiviert.

Als dauerhafte Schutzmaßnahme prüft der Auftragstestlauf nun die Plausibilität von PLZ und Ort. Steht im Feld PLZ Text, während das Feld Ort ausschließlich Ziffern enthält, wird der Auftrag hart blockiert; dieselbe Prüfung ist in die tägliche Kunden-/Rechnungsanschrift-Konsistenzprüfung eingebunden. Zum Zeitpunkt der Einführung betraf dies noch zwei aktive Rechnungstool-Aufträge: `5602` / Kunde `6342` und `5926` / Kunde `6426`. Diese Stammdaten werden nicht automatisch verändert, sondern müssen fachlich geprüft werden.
...
Die Rechnungsablage ist technisch umgesetzt und von MIDAS getrennt. Neue PDF-, XML- und spätere DOCX-Dateien werden ausschließlich unter `\\janus\Rechnungen` gespeichert; die Webapp sieht denselben Ordner als `/mnt/rechnungen`. Verbindliche PDFs verwenden aktuell das Schema `Jahr/Rechnungsnummer.pdf`. Die Samba-Freigabe und der Laravel-Speicherpfad wurden jeweils mit Schreib-, Lese- und Löschtest geprüft. Die bestehende MIDAS-Freigabe bleibt ausschließlich read-only für historische Rechnungen.
...
Bewusst blockiert werden derzeit E-Rechnungen mit negativen Dokumentzeilen, solange diese nicht eindeutig als EN16931-Zuschlag/Rabatt modelliert sind, sowie Positionen mit 0 % Umsatzsteuer ohne eindeutig hinterlegten Steuerbefreiungsgrund. An diesen Stellen werden keine steuerlichen Bedeutungen aus Alt-Daten geraten. Die historische Rechnungsanschrift enthält außerdem kein eigenes Länderfeld; die derzeitige E-Rechnungsausgabe setzt daher deutsche Rechnungsanschriften voraus. XRechnung mit Lastschrift wird ebenfalls vor der Ausgabe blockiert, solange Mandatsreferenz und Gläubiger-ID nicht eindeutig aus den produktiven Daten bereitgestellt werden können.

Im Adminbereich steht unter **Rechnungstool-Systemtest → E-Rechnung** ein realer read-only Prüflauf bereit. Er sucht echte fakturierbare Aufträge, erzeugt ZUGFeRD und XRechnung nur temporär und führt die vorhandenen Validatoren aus. Der bestätigte September-Lauf vom 19.09.2026 erzeugte für ZUGFeRD ein vollständig valides EN16931-PDF/A-3u; XML-Schema, semantische Prüfung, eingebettetes XML und veraPDF 1.30.2 waren erfolgreich. Beim realen XRechnungsfall mit Leitweg-ID `992-01497-46` zeigte KoSIT dagegen korrekt die fehlenden Direct-Debit-Pflichtangaben Mandatsreferenz, Gläubiger-ID und belastete Konto-ID. Daraufhin wurde die XRechnungs-Readiness zusätzlich gehärtet, sodass Lastschrift-XRechnungen bis zur eindeutigen Datenquelle bereits vor der Ausgabe blockiert werden.
...
Im Adminbereich steht innerhalb des **Rechnungstool-Systemtests** zusätzlich **Mahnwesen testen** zur Verfügung. Der read-only Endtest verwendet ausschließlich reale Bestandsfälle. Er prüft fällige Mahnstufen 1, 2 und 3 einschließlich offenem Hauptbetrag, vorhandenen Mahngebühren und Verzugszinsen, eine strittige Rechnung ohne automatische Eskalation, eine fällige Wiedervorlage, eine offene Ratenzahlungsrechnung, eine aktive Kundensperre sowie die Erzeugung eines echten Mahnschreiben-PDFs. Zusätzlich ruft der Test kontrolliert eine produktive Schreibmethode auf und erwartet, dass `DUNNING_WRITES_ENABLED=false` den Vorgang vor jedem SQL-Write blockiert. Der bestätigte Lauf vom 19.09.2026 bestand alle neun Prüfungen.

### Rollback und Notfallverfahren

Im selben Adminblock wird der verbindliche **Rollback-&-Notfallablauf** angezeigt. Die Formulierung „Schreibschalter deaktivieren“ bezieht sich ausschließlich auf einen späteren Produktivbetrieb. Solange `INVOICE_WRITES_ENABLED=false` und `DUNNING_WRITES_ENABLED=false` bereits gelten, ist keine Umschaltung erforderlich. Beide Flags sind auf Janus zusätzlich explizit in `/srv/dbapp/app/.env` hinterlegt.

Bei einem schweren Produktionsfehler werden beide Flags auf `false` gesetzt, anschließend unter `/srv/dbapp` der Laravel-Konfigurationscache mit `docker compose exec -T app php artisan config:clear` geleert und der App-Container mit `docker compose restart app` neu gestartet. Danach werden keine weiteren Rechnungs- oder Mahnoperationen ausgeführt, bis der konkrete Fehler abgegrenzt ist.

Die produktive Rechnungserzeugung läuft vollständig innerhalb einer SQL-Transaktion. Rechnungsnummer, `tblRechnung`, `tblAuftragPosBerechnet` und Änderungen an `tblAccountingKonto` werden gemeinsam zurückgerollt, wenn innerhalb des Commit-Ablaufs ein Fehler auftritt. Bereits erzeugte PDF- oder XRechnung-Dateien werden im Fehlerpfad wieder aus `\\janus\Rechnungen` gelöscht. Kann ein Dokument nicht entfernt werden, wird dies als `Invoice rollback could not remove generated document` im Laravel-Log protokolliert und muss manuell geprüft werden.

Vor jeder manuellen Datenkorrektur wird zunächst eine frische CARDEA-Sicherung angelegt. Eine einzelne fehlerhafte Rechnung ist grundsätzlich gezielt zu korrigieren; ein vollständiger Datenbank-Restore ist nur für einen größeren Datenbankschaden vorgesehen und erfolgt ausschließlich in einem Wartungsfenster bei gestoppten Schreibzugriffen. Nach Restore oder manueller Korrektur werden Rechnungszähler, betroffene Rechnungs-/Positionsdatensätze und die Dateiablage geprüft, bevor die jeweils betroffene Schreibfunktion bewusst wieder freigegeben wird.
...
Die frühere HTML-PDF-Vorlage bleibt vorerst nur als technische Altimplementierung im Projekt, wird aber weder für die PDF-Vorschau noch für die produktive Rechnungserzeugung verwendet.

Für mehrseitige Rechnungen wurde die Word-Vorlage zusätzlich gehärtet: Die Tabellenkopfzeile ist als wiederholte Kopfzeile markiert, Positions- und Summenzeilen dürfen nicht über einen Seitenumbruch zerrissen werden, und mehrzeilige Positionsbeschreibungen sowie zusätzliche Rechnungshinweise werden mit echten Word-Zeilenumbrüchen erhalten. Ein Stresstest mit 24 dreizeiligen Positionen erzeugt eine dreiseitige Rechnung ohne Konvertierungsfehler. Der Firmen-/Bankfuß bleibt über die vorhandene Word-Fußzeile auf den Seiten erhalten; der Geschäftsführertext wurde bereits so umgebrochen, dass `Tamás Lányi` nicht mehr außerhalb des rechten Seitenrandes liegt.

Die historische Artikelzeile zwischen Positionsnummer und MwSt. ist im Layout ebenfalls wieder vorhanden. Die konkrete Quelle der in der Referenzrechnung `2026001462` angezeigten Artikelnummer `(04412)` konnte jedoch weder in `tblProdukt`, `tblAuftragPos`, `tblAuftragPosBerechnet` noch in ähnlich benannten Artikel-/Produktfeldern bestätigt werden. Die Rechnungserzeugung unterstützt deshalb bereits ein optionales `articleNumber` pro Dokumentzeile, lässt die Artikelzeile aber leer, solange die Altquelle nicht eindeutig identifiziert ist. Janus erfindet an dieser Stelle bewusst keine Zuordnung aus interner Produkt-, Positions- oder DATEV-ID.
