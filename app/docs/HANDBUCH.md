...
Der Adminbereich enthält zusätzlich **Datenbank-Backup**. Von dort können `accountings`, `domains` und `topsnetdb_safe` einzeln oder gemeinsam zur Sicherung eingeplant werden. Die Ausführung erfolgt über einen eigenen Hintergrund-Worker, damit lange SQL-Backups nicht an einem Browser-Timeout scheitern. Die Sicherungen werden auf Janus unter `/mnt/backups` abgelegt und über die getrennte SMB-Freigabe `\\janus\Backups` bereitgestellt; für den Zugriff gilt derselbe Samba-Benutzer `rechnungen`. Große Datenbanken werden in mehrere `.bak`-Teile gestript und erhalten zusätzlich eine `manifest.json`, damit eindeutig dokumentiert ist, welche Dateien zusammengehören. Die Oberfläche prüft SQL-Rechte, freien Speicherplatz, letzte Full-Backup-Größe und Status der Hintergrundwarteschlange, bevor ein Start möglich ist. Derzeit stehen auf Janus rund 29 GB frei; die letzte Vollsicherung von `accountings` lag bei rund 82,3 GB. Deshalb bleibt `accountings` automatisch gesperrt, bis die Backup-Ablage auf ein größeres Volume verschoben oder Janus erweitert wurde. `domains` und `topsnetdb_safe` passen kapazitiv bereits auf die Freigabe.
