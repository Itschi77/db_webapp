...
Für Dateisystemprofile können Modus, lokaler Mount, UNC-Zielpfad und Dateinamenschema gepflegt werden. `storage.midas` bleibt die read-only eingebundene Altfreigabe. `storage.midas_invoices` ist die getrennte schreibbare Rechnungsablage; ihr Test schreibt eine temporäre Datei, prüft sie und entfernt sie wieder. Das Schema unterstützt die Platzhalter `{year}` und `{invoice_number}`. Die Verbindungen zu `accountings`, `domains`, `topsnetdb_safe`, beide MIDAS-Profile sowie das zunächst inaktive Grundprofil `smtp.primary` sind hinterlegt.
