...
Über **zum Kunden** werden Domains ohne gesetzte Kundenzuordnung einem vorhandenen Kunden zugeordnet. Links werden die noch nicht zugeordneten Domains aus `domains.dbo.tblDomains` angezeigt, rechts die Kunden aus `topsnetdb_safe.dbo.tblKunde` mit der jeweils jüngsten Auftragsposition aus `accountings`. Beide Bereiche besitzen eine Suche.
...
Der Pfad `/admin` ist ausschließlich für Mitglieder der zusätzlichen AD-Gruppe `DB-Webapp-Admins` erreichbar. Der Bereich verwaltet Verbindungsprofile für SQL Server, PostgreSQL, SMTP, Dateisysteme und HTTP-/HTTPS-Dienste. Bestehende Profile können bearbeitet und direkt getestet werden; Passwörter und Secrets werden verschlüsselt gespeichert und nach dem Speichern nicht wieder angezeigt. Änderungen werden erst nach einem erfolgreichen Verbindungstest für die Laufzeitkonfiguration berücksichtigt. Außerdem können vorhandene Laravel-Anwendungslogs mit den jeweils letzten 500 Zeilen eingesehen werden.

Für SMTP-Profile können Server, Port, Postfach/Anmeldename, verschlüsseltes Passwort, Verschlüsselungsart, Anmeldung mit Postfach/Passwort oder ohne Authentifizierung, Absenderadresse und Absendername gepflegt werden. **Erreichbarkeit testen** prüft nur die TCP-Verbindung. **Testmail senden** führt nach einer Sicherheitsabfrage eine echte Anmeldung und einen echten Versand aus; als Testempfänger ist `sw@tops.net` vorbelegt. Erfolg oder Fehler werden am Profil und im Anwendungslog festgehalten. Nur das erfolgreich getestete aktive Profil mit dem Schlüssel `smtp.primary` wird für den späteren Rechnungsversand verwendet.

Für Dateisystemprofile können Modus, lokaler Mount, UNC-Zielpfad und Dateinamenschema gepflegt werden. `storage.midas` bleibt ausschließlich als read-only eingebundene Altfreigabe für historische Rechnungen bestehen. Neue Rechnungsdokumente werden ausschließlich über `storage.rechnungen` nach `/mnt/rechnungen` beziehungsweise `\\janus\Rechnungen` geschrieben. Die Freigabe liegt lokal auf Janus unter `data/rechnungen` und wird über einen eigenen Samba-Docker-Service bereitgestellt. Der SMB-Zugriff erfolgt authentifiziert mit dem gemeinsamen Benutzer `rechnungen`; anonymer Gastzugriff ist deaktiviert. Das Kennwort liegt ausschließlich lokal in `secrets/samba.env` auf Janus und wird nicht versioniert. Das Schema unterstützt die Platzhalter `{year}` und `{invoice_number}`; aktuell wird `{year}/{invoice_number}.pdf` verwendet. Ein Schreibtest prüft Dateiablage, Rücklesen und Löschen. MIDAS ist für neue PDF-, XML- oder DOCX-Dateien kein Schreibziel mehr.

Im Adminbereich ist die Backupfunktion direkt in die Sektion **Datenbanken** integriert. Dort können `accountings`, `domains` und `topsnetdb_safe` einzeln oder gemeinsam zur Sicherung eingeplant werden. Die Ausführung erfolgt über einen eigenen Hintergrund-Worker, damit lange SQL-Backups nicht an einem Browser-Timeout scheitern. Die `.bak`-Dateien werden ausschließlich direkt auf CARDEA im vom SQL Server gemeldeten Standard-Backupordner erzeugt; eine Kopie nach Janus findet nicht statt. Die Adminseite zeigt je Datenbank Größe, Backuprecht, CARDEA-Zielpfad und letzte Vollsicherung an. Die Historie stammt direkt aus `msdb.dbo.backupset` und `msdb.dbo.backupmediafamily`. Für die Funktion wird nur `BACKUP DATABASE` je Datenbank benötigt.

## 36. Rechnungstool und noch nicht vollständig migrierte Bereiche

Der Menüpunkt **Rechnungstool starten** ist für Benutzer mit der zusätzlichen AD-Berechtigung `DB-Webapp-Rechnungstool` aktiv und öffnet den geschützten Bereich `/fakturierung`. Ohne diese Zusatzberechtigung bleibt der Menüpunkt deaktiviert; ein direkter Aufruf des Pfads wird serverseitig mit HTTP 403 abgewiesen. Phase 1 ist rein lesend und enthält inzwischen eine echte Auftragsliste mit Zeitraum, den Auswahlarten **Nachträglich**, **Im Voraus** und **Domainaufträge**, Suche sowie eine Vorschau der Auftragspositionen und ihrer bisherigen Berechnungsstände. Normale Festpreispositionen sowie die Staffeltypen 1 bis 7 können bereits im Testlauf berechnet werden. Dazu gehören Staffel, Linearstaffel, Zeittarif, Bandbreite, Domainabrechnung und Bereichsstaffel. Über **Accountings berücksichtigen** wird wie im Alttool gesteuert, ob Accountingpositionen einbezogen oder übersprungen werden. Vorberechnung/Nachberechnung wird inzwischen ebenfalls lesend simuliert: aktive Vorberechnungspositionen werden auf den Folgemonat bezogen und Staffelwechsel zeigen Erstattung bzw. Nachberechnung auf Basis des bisherigen `tblAccountingKonto`-Stands. Für einen ausgewählten Auftrag erzeugt die moderne Ansicht außerdem einen vollständigen read-only Auftragstestlauf mit Empfänger, Zahlungsbedingung, DATEV-Prüfungen, Rechnungszeilen, Skonto sowie Netto-, Steuer- und Bruttosummen. Blockierende Inkonsistenzen werden vor einer späteren produktiven Rechnungserzeugung sichtbar gemacht. Die normale Bedienoberfläche schreibt weiterhin keine Rechnungen oder Abrechnungsdaten, solange `INVOICE_WRITES_ENABLED=false` gesetzt ist. Die transaktionssichere Schreiblogik einschließlich PDF-Ablage ist technisch vorbereitet und wurde ausschließlich in kontrollierten Testläufen geprüft. Zusätzlich stehen Kunden- und Gesamttestläufe zur Verfügung. Ein Kundenlauf verarbeitet alle passenden Aufträge der angegebenen Kundennummer, ein Gesamtlauf alle passenden Aufträge des gewählten Zeitraums und Abrechnungstyps. Jeder Auftrag wird unabhängig geprüft, sodass blockierte Aufträge die übrigen Prüfungen nicht stoppen. Die Übersicht trennt fakturierbare, bereits erledigte und blockierte Aufträge; Geldsummen werden nur aus fakturierbaren Aufträgen gebildet. Unterhalb des Rechnungstools steht zusätzlich die Box **Manuell prüfen**. Sie führt die nach dem aktuellen DB-Abgleich noch bekannten Auffälligkeiten mit Auftrag, Kunde, Kategorie und Prüfhinweis auf. Die Auftragsnummer ist direkt anklickbar. Die Box ist nur eine Kontrollliste und ändert keine Daten.

Der Adminbereich enthält außerdem den **Rechnungstool-Systemtest** als gebündelte read-only Abnahme vor der Produktivfreigabe. Der Lauf sucht automatisch repräsentative Echtfälle für Festpreis, Voraus, Staffel/Accounting, Domain, mehrere Dokumentpositionen und Rabatt, rendert erfolgreiche Fälle temporär als PDF und prüft die Rechnungsablage, den MIDAS-Schreibschutz, den deaktivierten Produktivschalter sowie die PDF-Konvertierung. Er gehört bewusst nicht zur täglichen Fakturierungsoberfläche. Der bestätigte Lauf vom 19.09.2026 bestand alle sechs Fallklassen und alle vier Systemprüfungen. Im selben Adminblock stehen zusätzlich **E-Rechnung testen**, **Mahnwesen testen** sowie **Rollback & Notfall**. Der E-Rechnungstest prüft reale ZUGFeRD-/XRechnungs-Fälle mit XSD/EN16931, KoSIT und PDF/A-3u; ZUGFeRD ist real erfolgreich validiert, während XRechnung mit Lastschrift bis zur eindeutigen Bereitstellung von Mandatsreferenz und Gläubiger-ID gesperrt bleibt. Der Mahnwesen-Endtest prüft reale fällige Mahnstufen 1 bis 3, strittige Rechnungen, Wiedervorlage, Ratenzahlung, Kundensperre, Mahn-PDF und den serverseitigen Schreibschutz. Der bestätigte Lauf vom 19.09.2026 bestand alle neun Mahnwesen-Prüfungen. Der Notfallbereich zeigt den aktuellen Status beider Produktiv-Flags und die verbindliche Abschalt-, Prüf-, Sicherungs-, Rollback- und Wiederfreigabe-Reihenfolge für einen späteren Produktivbetrieb.
...
Alle fünf Dokumentationsbereiche besitzen oben die Schaltflächen **PDF exportieren** und **Drucken**. **PDF exportieren** erzeugt serverseitig eine A4-PDF-Datei mit Titel, Erzeugungsdatum und Seitenzahlen und lädt sie herunter. **Drucken** öffnet die normale Browser-Druckfunktion; Navigation und Suchleiste werden dabei automatisch ausgeblendet. Verfügbar ist dies für Technische Dokumentation, Benutzerhandbuch, Handbuch Rechnungstool, SQL-Statement-Wiki und das fortlaufende Testprotokoll mit Fehleranalyse.

## 38. Dokumentationsstand

Im Classic-Hauptmenü stehen links fünf Direktbuttons für **Technische Doku**, **Benutzerhandbuch**, **Handbuch Rechnungstool**, **SQL-Statement-Wiki** und **Testprotokoll** bereit. Im modernen Frontend stehen dieselben fünf Direktbuttons oben rechts. Alle fünf öffnen die jeweilige Dokumentation in einem neuen Browser-Tab. Das Testprotokoll hält Abnahmeläufe, Fehleranalyse und Behebungsmaßnahmen fortlaufend fest. Das separate Rechnungstool-Handbuch beschreibt ausschließlich den Bedienablauf der migrierten Fakturierung; technische Details bleiben in der Technischen Doku und SQL bleibt im gemeinsamen SQL-Statement-Wiki.
